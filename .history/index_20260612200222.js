...
d3.csv("data/nba.csv",d3.autoType).then(function(data){
   ...
   d3.select("#selectPlayer").on("change",updateVisualization1(data))
   d3.select("#seasonSelect").on("change",updateVisualization1(data))
   d3.select("#metricSelect").on("change",updateVisualization1(data))
...
function updateVisualization1(data){
    ...
    const data_filter = data.filter(d => d.player_name === player && d.season_year === season)
