...
function draw_Rows(svg,new_data,left,right,margin,r_height,scale){
    ...
    rows.append("circle").attr("cx",left).attr("cy",35).attr("r",2).attr("r" ,d=>scale(d.current_v)).attr("fill","black")
    rows.append("circle").attr("cx",right).attr("cy",35).attr("r",2).attr("r" ,d=>scale(d.next_v)).attr("fill","black")
