export function drawVisualization3(data,id,metric,threshold){
     ...
     console.log(baseline_classif.classification.length)
...
function addLablels(svg, classification,canvas){
    ...
    .attr("x",(d,j) => j * 20 + 10 /2).attr("y",75).attr("text-anchor","middle")
...
function addCells(svg, classification,canvas,scale){
    ...
    .attr("x",(d,j) => canvas.margin.left + j * 20).attr("y",85).attr("width",7).attr("height",7)
