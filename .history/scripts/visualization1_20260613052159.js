...
function addLegends(svg,canvas){
    const lg = svg.append("g").attr("transform",
        `translate(${canvas.width - canvas.margin.right+20},${canvas.margin.top})`)


    lg.append("circle").attr("cx",0).attr("cy",0).attr("r",2).attr("fill","#d94801")
    lg.append("text").attr("x",5).attr("y",0).text("Hot game").attr("font-size","5px")

    lg.append("circle").attr("cx",0).attr("cy",30).attr("r",2).attr("fill","#999")
    lg.append("text").attr("x",15).attr("y",35).text("Normal game").attr("font-size","5px")

    lg.append("circle").attr("cx",0).attr("cy",60).attr("r",2).attr("fill","#3182bd")
    lg.append("text").attr("x",15).attr("y",65).text("Cold game").attr("font-size","5px")
}