...
function createCanvas(){
    return{
        width : 500, height: 250, margin:{top: 20,right:40, bottom:60, left:50}
    }
}
...
function addLegends(svg){
    ...
    lg.append("circle").attr("cx",120).attr("cy",0).attr("r",2).attr("fill","#999")
    lg.append("text").attr("x",135).attr("y",5).text("Normal game")

    g.append("circle").attr("cx",270).attr("cy",0).attr("r",2).attr("fill","#3182bd")
    lg.append("text").attr("x",200).attr("y",5).text("Cold game")
}