export function drawVisualization2(data,id,metric,eventT){
    if(data.length === 0) {
     ...
     draw_Header(svg,280,480,metric)
}
...
function draw_Header(svg,left,right,metric){
    ...
    .attr("text-anchor","middle").attr("font-weight","bold").text("Game n")

    svg.append("text").attr("x",right).attr("y",30)
    .attr("text-anchor","middle").attr("font-weight","bold").text("Game n+1")

    svg.append("text").attr("x",120).attr("y",30)
    .attr("font-weight","bold").text(`Metric : ${metricName(metric)}`)
}
