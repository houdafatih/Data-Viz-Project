...
function addAverageLine(svg,scales,data,metric){
    ...
    console.log(data.map(d => d.rolling_pts_avg_3))
...
function highlightGameandNextGame(svg,data,selected){
    const nextG = data.find(d => d.game_index === selected.game_index + 1)

    svg.selectAll(".game-point").attr("r",8).attr("stroke","none").attr("opacity",0.4)

    svg.selectAll(".game-point").filter(d => d.game_index === selected.game_index)
    .attr("stroke","black").attr("stroke-width",3).attr("r",5).attr("opacity",1)

    if(nextG){
        svg.selectAll(".game-point").filter(d => d.game_index === nextG.game_index)
       .attr("stroke","#3182bd").attr("stroke-width",3).attr("r",8).attr("opacity",1)
    }
}