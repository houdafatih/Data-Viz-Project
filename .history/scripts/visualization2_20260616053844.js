...
function addSummaryCard(svg,new_data,eventT,canvas){
    ...
    const avg_pts = d3.mean(metrics_change,d => d.pts)
    const avg_fga = d3.mean(metrics_change,d => d.fga)
    const avg_min = d3.mean(metrics_change,d => d.min)
    const avg_fg_pct = d3.mean(metrics_change,d => d.fg_pct)

    svg.append("text").attr("x",canvas.margin.left).attr("y",canvas.height-40)
    .text(`PTS : ${avg_pts}  FGA : ${avg_fga}  MIN : ${avg_min}  FG_PCT : ${avg_fg_pct}`)
}