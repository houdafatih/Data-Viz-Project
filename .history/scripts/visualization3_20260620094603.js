export function drawVisualization3(data,id,metric,threshold){
     ...
     addTitle(svg,canvas,baseline_classif.baseline)
...
function addTitle(svg,canvas,baseline){
    svg.append("text").attr("x",canvas.margin.left).attr("y",25).attr("font-size","12px")
    .attr("font-weight","bold").text("Hot/Cold Streak Explorer")

    svg.append("text").attr("x",canvas.margin.left).attr("y",45).attr("font-size","12px")
    .text(`Metric: ${canvas.metric} | Baseline: ${baseline.toFixed(2)}`)
}
...
function addLablels(svg, classification,canvas){
    ...
    .attr("font-size","9px").text(d => `G${d.g_num}`)
}
