...
function draw_Rows(svg,new_data,left,right,margin,r_height,scale){
    ...
    rows.append("text").attr("x",30).attr("y",35).text(d =>`Event ${d.eventN}` ).attr("font-size","6px")
    ...
    .attr("y1",40).attr("y2",40).attr("stroke","black").attr("stroke-dasharray","5 3")
    .attr("stroke-width",1.5)

    rows.append("circle").attr("cx",left).attr("cy",40).attr("r",2).attr("r" ,d=>scale(d.current_v)).attr("fill","black")
    rows.append("circle").attr("cx",right).attr("cy",40).attr("r",2).attr("r" ,d=>scale(d.next_v)).attr("fill","black")
