...
function addPoints(svg,scales,data,metric){
    svg.selectAll("circle").data(data).enter()
     ...
     .attr("r",4).attr("fill",d => colorPoint(d.performance))
}
...
function addLabels(svg,canvas,metric){
    ...
    .attr("x",canvas.width/2).attr("y",canvas.height-10)
    .attr("text-anchor","middle").attr("text","Game Order")

    svg.append("text").attr("class","y-label")
    .attr("x",-canvas.height/2).attr("y",10).attr("transform","rotate(-90)")
    .attr("text-anchor","middle").attr("text",metricName(metric))
}
