...
function addSummaryCard(svg,new_data,eventT,canvas){
    ...
    const means = [
        ...
        {"avg":d3.mean(metrics_change,d => d.fg_pct),"name":"FG_PCT"}
    ]
    
    means.forEach((element,i) => {
        svg.append("text").attr("x",canvas.margin.left+i*50).attr("y",canvas.height-30)
        .attr("font-size","6px").attr("font-weight","bold")
        .text(`${element.name}`)   
        svg.append("text").attr("x",canvas.margin.left+i*70).attr("y",canvas.height-30)
        .attr("font-size","6px")
        .text(`${element.avg}`)
