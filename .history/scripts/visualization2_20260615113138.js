export function drawVisualization2(data,id,metric,eventT){
    if(data.length === 0) {
     ...
     draw_Rows(svg,new_data,200,400,canvas.margin,55)
     console.log(new_data)
}
function createCanvas(new_data){
    return{
        width : 600, height: 200 + new_data.length * 50 , margin:{top: 20,right:100, bottom:60, left:50}
    }
}
...
function draw_Rows(svg,new_data,left,right,margin,r_height){
    ...
    const rows = svg.selectAll(".response-row").data(new_data).enter()
     ...
     .attr("transform",(d,j) => `translate(0,${margin.top+j})`)

    rows.append("text").attr("x",100).attr("y",35).text(d =>`Event ${d.eventN}` ).attr("font-size","6px")
    ...
    rows.append("circle").attr("cx",left+10).attr("cy",35).attr("r",2).attr("fill","black")
    rows.append("circle").attr("cx",right-10).attr("cy",35).attr("r",2).attr("fill","black")

    rows.append("text").attr("x",right).attr("y",35).text(d => d.next_v).attr("font-size","6px")

    rows.append("text").attr("x",right+90).attr("y",35).text(d => difference_label(d.difference)).attr("font-size","6px")
